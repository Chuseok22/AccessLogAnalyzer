...
class AnalyzerService:
    ...
    def compare_security_and_overtime(self, security_status_by_day: Dict[date, List[Dict[str, str]]], 
                                    overtime_records: List[Dict[str, Any]]) -> List[SuspiciousRecord]:
        """
        ...
        for overtime in overtime_records:
            ...
            if not security_status:
                self._handle_no_security_record(
                    suspicious_records, business_date, employee_name, 
                    overtime_start, overtime_end, overtime_records
                )
                continue
            
            # 경비 기록이 있는 경우 분석
            result = self._analyze_security_overtime_overlap(
                business_date, employee_name, overtime_start, overtime_end, 
                security_status, overtime_records
            )
    ...
    def _handle_no_security_record(self, suspicious_records: List[SuspiciousRecord], 
                                 business_date: date, employee_name: str,
                                 overtime_start: time, overtime_end: time,
                                 overtime_records: List[Dict[str, Any]]) -> None:
        """
        경비 기록이 없는 경우를 처리합니다.
    ...
    def _analyze_security_overtime_overlap(self, business_date: date, employee_name: str,
                                       overtime_start: time, overtime_end: time,
                                       security_status: List[Dict[str, str]],
                                       overtime_records: List[Dict[str, Any]]) -> Optional[SuspiciousRecord]:
        """
        ...
        try:
            ...
            if overtime_end < overtime_start:
                overtime_end_dt = datetime.combine(
                    business_date + timedelta(days=1), overtime_end
                )
            ...
            if total_suspicious_hours > 0:
                return self._create_suspicious_record(
                    business_date, employee_name, overtime_start, overtime_end,
                    total_suspicious_hours, suspicious_periods, security_changes,
                    overtime_start_dt, overtime_end_dt, overtime_records
                )
    ...
    def _create_security_periods(self, security_changes: List[Dict[str, Any]], 
                ...
                if i == len(security_changes) - 1:
                    ...
                    if current["상태"] == "시작":
                        next_day = datetime.combine(
                            business_date + timedelta(days=1), time(0, 0)
                        )
                        security_periods.append(
    ...
    def _check_suspicious_intervals(self, overtime_start_dt: datetime, overtime_end_dt: datetime,
                                   security_periods: List[Dict[str, Any]], business_date: date,
                                   employee_name: str) -> List[Tuple[datetime, datetime]]:
        """
    ...
    def _create_suspicious_record(self, business_date: date, employee_name: str,
                                overtime_start: time, overtime_end: time,
                                total_suspicious_hours: float, suspicious_periods: List[str],
                                security_changes: List[Dict[str, Any]],
                                overtime_start_dt: datetime, overtime_end_dt: datetime,
                                overtime_records: List[Dict[str, Any]]) -> SuspiciousRecord:
        """
